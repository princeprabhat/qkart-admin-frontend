...
const Checkout = () => {
  ...
  const addAddress = async (token, newAddress) => {
    ...
    try {
      ...
      setNewAddress({isAddingNewAddress: false,
        value: "",});
          ...
          <Box className="shipping-container" minHeight="100vh">
             ...
             <Box>
              {addresses.all.map((item)=>{
                return(
                  ...
                  <Box  className={`address-item ${addresses.selected===item._id?"selected":"not-selected"}`} key={item._id} onClick={()=>{updateSelectedAddress(item._id)}} >
                    {item.address}
                    <Button startIcon={<Delete />} variant="text" onClick={()=>{deleteAddress(token,item._id)}}>
              DELETE
            </Button>
                  </Box>
                )
              ...
              {!addresses?.all && <Typography my="1rem">
           ...
           {!newAddress.isAddingNewAddress ? <Button
            ...
            <Button startIcon={<CreditCard />} variant="contained" onClick={()=>{
              if(validateRequest(items,addresses) && performCheckout(token,items,addresses)){
